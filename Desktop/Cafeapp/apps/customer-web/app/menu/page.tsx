...
export default function MenuPage() {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#FAF8F5', fontFamily:'Inter,sans-serif', color:'#2B1810' }}>
      ...
      {/* ══ DAILY SPECIALS STRIP ══ */}
      {dailySpecials.length > 0 && (
        <section style={{ padding:'36px 0 0', maxWidth:1180, margin:'0 auto' }}>
          <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:16, padding:'0 clamp(16px,4vw,60px)' }}>
            <div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:6, background:'rgba(181,122,60,0.1)', borderRadius:20, padding:'4px 12px', marginBottom:7 }}>
                <span style={{ fontSize:12 }}>✨</span>
                <span style={{ fontSize:10.5, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'#B57A3C' }}>Today Only</span>
              </div>
              <h2 style={{ fontFamily:'"Playfair Display",serif', fontSize:'clamp(19px,2.5vw,24px)', fontWeight:700, color:'#2B1810', letterSpacing:'-0.02em', lineHeight:1.1 }}>
                Chef's Daily Picks
              </h2>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 12px', background:'rgba(109,191,126,0.12)', borderRadius:20, border:'1px solid rgba(109,191,126,0.25)', flexShrink:0 }}>
              <span style={{ width:6, height:6, borderRadius:'50%', background:'#6DBF7E', display:'inline-block', animation:'pulse 2s ease-in-out infinite' }}/>
              <span style={{ fontSize:11.5, fontWeight:600, color:'#4F7A54' }}>Live today</span>
            </div>
          </div>
          <div style={{
            display:'flex', gap:14,
            overflowX:'auto', overflowY:'visible',
            padding:'4px clamp(16px,4vw,60px) 16px',
            ...
            {dailySpecials.map((s, idx) => (
              <div key={s.id} style={{
                ...
                boxShadow:'0 4px 16px rgba(43,24,16,0.07)',
                transition:'transform 0.25s, box-shadow 0.25s',
              }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform='translateY(-4px)'; (e.currentTarget as HTMLElement).style.boxShadow='0 12px 32px rgba(43,24,16,0.13)'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform='translateY(0)'; (e.currentTarget as HTMLElement).style.boxShadow='0 4px 16px rgba(43,24,16,0.07)'; }}
              >
                <div style={{
                  height:140,
                  ...
                  <div style={{ position:'absolute', top:9, left:9, display:'flex', alignItems:'center', gap:4, background:'rgba(181,122,60,0.92)', borderRadius:20, padding:'3px 9px', backdropFilter:'blur(6px)' }}>
                    <span style={{ fontSize:9 }}>⭐</span>
                    <span style={{ fontSize:10, fontWeight:700, color:'#FAF8F5', letterSpacing:'0.05em' }}>SPECIAL</span>
                  </div>
                  {!s.imageUrl && (
                    <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:46, opacity:0.5 }}>
                      {'☕🍰🥗🍵🍕'.split('').filter((_,i) => i === idx % 5)[0] || '🍽'}
                    </div>
                  )}
                </div>
                <div style={{ padding:'12px 14px 14px' }}>
                  <div style={{ fontSize:13, fontWeight:700, color:'#2B1810', marginBottom:4, lineHeight:1.3, fontFamily:'"Playfair Display",serif' }}>{s.title}</div>
                  {s.description && (
                    <div style={{ fontSize:11.5, color:'#9E7B6D', lineHeight:1.5, marginBottom:8,
                      display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' as any, overflow:'hidden' }}>
                      {s.description}
                    </div>
                  )}
                  <div style={{ fontFamily:'"Playfair Display",serif', fontSize:17, fontWeight:700, color:'#B57A3C' }}>₹{s.price}</div>
                </div>
              </div>
            ))}
          </div>
