  ...
  return (
    <div style={{ minHeight:'100vh', background:'#FAF8F5', fontFamily:'Inter,sans-serif', color:'#2B1810' }}>
      ...
      {dailySpecials.length > 0 && (
        <section style={{ padding:'52px 0 0', maxWidth:1180, margin:'0 auto' }}>
          {/* Header */}
          <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:20, padding:'0 clamp(20px,4vw,60px)' }}>
            <div>
              <div style={{ display:'inline-flex', alignItems:'center', gap:6, background:'rgba(181,122,60,0.1)', borderRadius:20, padding:'4px 12px', marginBottom:8 }}>
                <span style={{ fontSize:13 }}>✨</span>
                <span style={{ fontSize:10.5, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'#B57A3C' }}>Today Only</span>
              </div>
              <h2 style={{ fontFamily:'"Playfair Display",serif', fontSize:'clamp(22px,3vw,28px)', fontWeight:700, color:'#2B1810', letterSpacing:'-0.02em', lineHeight:1.1 }}>
                Chef's Daily Picks
              </h2>
              <p style={{ fontSize:13.5, color:'#9E7B6D', marginTop:5 }}>
                {new Date().toLocaleDateString('en-IN', { weekday:'long', day:'numeric', month:'long' })}
              </p>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 14px', background:'rgba(109,191,126,0.12)', borderRadius:20, border:'1px solid rgba(109,191,126,0.25)', flexShrink:0 }}>
              <span style={{ width:7, height:7, borderRadius:'50%', background:'#6DBF7E', display:'inline-block', animation:'pulse 2s ease-in-out infinite' }}/>
              <span style={{ fontSize:12, fontWeight:600, color:'#4F7A54' }}>Available now</span>
            </div>
          </div>
          ...
          <div style={{
            ...
            padding:'4px clamp(20px,4vw,60px) 20px',
            scrollSnapType:'x mandatory',
            WebkitOverflowScrolling:'touch' as any,
            msOverflowStyle:'none',
          }}>
            {dailySpecials.map((s, idx) => (
              <div
                key={s.id}
                onClick={() => router.push('/menu')}
                style={{
                  ...
                  boxShadow:'0 4px 18px rgba(43,24,16,0.07)',
                  position:'relative',
                }}
                onMouseEnter={e => { (e.currentTarget as HTMLElement).style.transform='translateY(-4px)'; (e.currentTarget as HTMLElement).style.boxShadow='0 12px 36px rgba(43,24,16,0.14)'; }}
                onMouseLeave={e => { (e.currentTarget as HTMLElement).style.transform='translateY(0)'; (e.currentTarget as HTMLElement).style.boxShadow='0 4px 18px rgba(43,24,16,0.07)'; }}
              >
                {/* Image / placeholder */}
                <div style={{
                  height:160,
                  background: s.imageUrl
                    ...
                    : `linear-gradient(135deg, hsl(${(idx * 47 + 22) % 360},35%,28%), hsl(${(idx * 47 + 52) % 360},40%,38%))`,
                  position:'relative',
                }}>
                  {/* "Special" badge */}
                  <div style={{ position:'absolute', top:10, left:10, display:'flex', alignItems:'center', gap:5, background:'rgba(181,122,60,0.92)', borderRadius:20, padding:'4px 10px', backdropFilter:'blur(6px)' }}>
                    <span style={{ fontSize:10 }}>⭐</span>
                    <span style={{ fontSize:10.5, fontWeight:700, color:'#FAF8F5', letterSpacing:'0.05em' }}>SPECIAL</span>
                  </div>
                  {/* No-image emoji fallback */}
                  {!s.imageUrl && (
                    <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:52, opacity:0.55 }}>
                      {'☕🍰🥗🍵🍕'.split('').filter((_,i) => i === idx % 5)[0] || '🍽'}
                    </div>
                  )}
                </div>
                ...
                <div style={{ padding:'14px 16px 16px' }}>
                  <div style={{ fontSize:13.5, fontWeight:700, color:'#2B1810', marginBottom:5, lineHeight:1.3, fontFamily:'"Playfair Display",serif' }}>
                    {s.title}
                  </div>
                  {s.description && (
                    <div style={{ fontSize:12, color:'#9E7B6D', lineHeight:1.55, marginBottom:10,
                      display:'-webkit-box', WebkitLineClamp:2, WebkitBoxOrient:'vertical' as any, overflow:'hidden' }}>
                      {s.description}
                    </div>
                  )}
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                    <div style={{ fontFamily:'"Playfair Display",serif', fontSize:18, fontWeight:700, color:'#B57A3C' }}>
                      ₹{s.price}
                    </div>
                    <button
                      onClick={e => { e.stopPropagation(); addItem({ menuItemId: s.id, name: s.title, price: s.price, quantity: 1 }); }}
                      style={{ padding:'6px 14px', borderRadius:10, background:'linear-gradient(135deg,#3E2723,#5D4037)', color:'#FAF8F5', border:'none', cursor:'pointer', fontSize:12, fontWeight:600, fontFamily:'Inter,sans-serif', transition:'all 0.2s' }}
          ...
          <style>{`.daily-strip::-webkit-scrollbar{display:none}`}</style>
        </section>
      )}
